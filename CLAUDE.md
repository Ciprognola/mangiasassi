...
## SESSION HANDOFF
F4b1 (0.4_11) approved. 0.4_12 (F4b2: cutter/margin-anchoring fix, exporting the previewed skin as a developer submission package via `expSpriteItems()`, `DEV_SKIN_TEST` fully removed) built, awaiting the owner's phone test. Next: REQ (the tile-10 reward skin, made with the tool once the owner supplies the art) or F2 (player login + cloud save, flag off) — brief arrives from a new Claude chat. Sessions may run as Claude Code cloud sessions: then the brief starts with a cloud preamble; follow it.
