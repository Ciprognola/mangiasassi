...
**State:** `VERSION` = `0.3_35`, everything pushed to `main`, `git status` clean (the `VERSION` file in the repo root is a stale old list: `index.html` is the source of truth).
**Done through M8a** (§10.8): M0-M7 + M8a. The random encounter is live: `bjAfterClear` (El Gamblador's gap) also rolls Ferma Algidone!, `faInvite` -> `startFerma({run:true,encounter:N})` -> Continua returns to the maze; Giochi card G4 + unlock popup + `S.p.fa={enc,seen}` + dev buttons.
**Waiting for the owner:** phone test of 0.3_35 (invite, encounter 1/2/3 floors, Continua back to the maze, the interlude after a forced girone-10 encounter, the Giochi card + popup, dev "Incontro N piani" / "Forza incontro").
**Remaining to 0.4:** **M8b** (rewards: sordi/exp paid ONCE on encounter win, existing formula x N floors required; the 3 Minigiochi achievements "Fuori da Coccia!" / "Algidone è a terra" / "Senza un graffio" and roadmap tile 10 ("Premio in arrivo") fire when the feat happens, even if the encounter is lost later;
the hooks are `FA.enc` (n, run) in `faWinPanel`/`faFinalPanel`/`faOver`/`faExit`) → **M9** (mobile controls polish + balance of every tunable in §10.9) → **REL**
...
| M8a | Integration part 1: entry (random encounter), encounter rule, Giochi card, unlock popup, save migration, dev buttons | P1, M7; six §10.9 proposals (confirmed 2026-09-25) | Sonnet · medium | done (0.3_35), awaiting phone test |
| M8b | Integration part 2: rewards (sordi/exp once on encounter win, x floors required), the 3 Minigiochi achievements, roadmap tile 10 as "Premio in arrivo" | M8a | Sonnet · medium | todo |
