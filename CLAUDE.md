...
**State:** `VERSION` = `0.3_36`, everything pushed to `main`, `git status` clean (the `VERSION` file in the repo root is a stale old list: `index.html` is the source of truth).
**Done through M8b** (§10.8): M0-M8b. Ferma Algidone! is integrated: random encounter (`bjAfterClear` → `faInvite` → `startFerma({run:true,encounter:N})`), Continua back to the maze, Giochi card G4 (practice) + popup, `S.p.fa={enc,seen,f1}`;
M8b: an encounter win adds its final score to `G.score` (`faEncPay`, paid at the last floor's panel), 3 Minigiochi achievements fire on floor clear (`faClear` → `ach("faclear",...)`, real encounters only via `faReal()`), roadmap tile 10 unlocks on `S.p.fa.f1` (first real floor-1 clear), "Premio in arrivo".
**Waiting for the owner:** phone test of 0.3_36 (score added after an encounter win and converted at run end, the three achievements + popup during the climb, tile 10 locked → unlocked with "Premio in arrivo", nothing paid on loss/practice).
**Remaining to 0.4:** **M9** (mobile controls polish + balance of every tunable in §10.9, incl. encounter score vs girone score and the achievement rewards) → **REL**
(0.4 release: VERSION "0.4", CHANGELOG, README, DEVELOPERS.md incl. the `sound.fa.<slot>` targets listed in §10.9, roll base to 0.4 in §6/§7/§9, tag `v0.4`). Audio submissions A1…An are post-0.4 (§8), not a dependency. The 0.5 backlog (§8) is NOT to be touched now.
