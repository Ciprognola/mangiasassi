...
- **Asset sheets** come from the owner, one per skin, when the chunk asks. Cut them with `tools/cut_sprites.py`.
- **Missing frame rule**: if a direction/frame is missing from the sheet, **print a WARNING listing the missing
  ...
  without the skin.
- **K2 GEKA SNC hat** — Uomo roccia, a hat with the text "GEKA SNC".
- **K3 Kebab costume** — Algidone.
