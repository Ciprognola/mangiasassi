...
## SESSION HANDOFF
F4c (0.4_10) approved, with two sheet fixes folded into 0.4_11 (streaks split out of the baked boar frames; label sizes on the ×4 sheets). 0.4_11 (F4b1: sheet fixes + «Parti da» on the sheet export + «Carica costume» import + live local preview) built, awaiting the owner's phone test. Next: F4b2 (export the imported skin as a developer submission package; remove `DEV_SKIN_TEST`), brief arrives from a new Claude chat. Sessions may run as Claude Code cloud sessions: then the brief starts with a cloud preamble; follow it.
...
| F4b2 | Skin tool part 2b: export the imported/staged skin as a developer submission package; remove `DEV_SKIN_TEST` | F4b1 | Sonnet · high | todo |
