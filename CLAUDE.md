...
## SESSION HANDOFF
F2a (0.4_13) approved. 0.4_14 (F2b: cloud save, flag off — `CLOUD_SAVE=false`, testable only on the dev site through «Salvataggio cloud (test)») built, awaiting the owner's phone test; the owner must paste the new `saves` rules block (`firestore.rules`) into the console before testing. Next: REQ (tile-10 reward skin; the resized-sheet import check folds into it), B1 (maze hitboxes), REL. REQ and B1 stay wait-assets until the owner's art/screenshot arrives. Sessions may run as Claude Code cloud sessions: then the brief starts with a cloud preamble; follow it.
...
| F2b | Cloud save, flag off (`CLOUD_SAVE`) — `saves/{uid}`, rev transactions, conflict popup, backup | F2a | Sonnet · high | done (0.4_14) |
