...
**State:** `VERSION` = `0.3_36` (unchanged: M9a is a report, no code), everything pushed to `main`, `git status` clean (the `VERSION` file in the repo root is a stale old list: `index.html` is the source of truth).
**Done through M8b** (§10.8) + **M9a report**: `refs/ferma_algidone/M9_BALANCE.md` (tables of every tunable + controls at 360×640 / 360×800 / 390×844, screenshots in `refs/ferma_algidone/review/m9_*`).
Key findings: Ferma ignores the Facile/Media/Difficile setting (only the payout scales); ~80 % of an encounter score is the stock bonus; pause button and Ferma panel/invite/Gioca buttons are under 44 px (38/38/30 tall); nothing overlaps on the three viewports.
**Waiting for the owner:** the picks on the report (section 3 lists 6 yes/no items; suggested default = all "safe" items + keep every balance row). Then **M9b** builds exactly those picks (bump `0.3_37`).
**Remaining to 0.4:** **M9b** → **REL** (0.4 release: VERSION "0.4", CHANGELOG, README, DEVELOPERS.md incl. the `sound.fa.<slot>` targets listed in §10.9, roll base to 0.4 in §6/§7/§9, tag `v0.4`). Audio submissions A1…An are post-0.4 (§8), not a dependency. The 0.5 backlog (§8) is NOT to be touched now.
...
| M9b | Build the owner's picks from the M9a report (mobile controls polish + balance) | M9a picks | Sonnet · medium | todo |
